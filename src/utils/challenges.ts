export default [
  {
    name: "Machine Learning",
    description:
      "Machine Learning Machine Learning Machine Learning Machine Learning",
    icon: {
      src: "mlIcon",
      imgStyle: "w-16",
      initialStyle: "-translate-x-6 translate-y-8",
      translatedStyle: "-translate-x-32 translate-y-8",
    },
    dark: true,
    secondaryColor: {
      topBarBgColor: "bg-yellow",
      actionTextColor: "text-yellow",
    },
    logos: [
      {
        src: "mlLogo",
        imgStyle: "w-20",
        initialStyle: "translate-x-12 -translate-y-8",
        translatedStyle: "translate-x-20 -translate-y-16",
      },
    ],
  },
  {
    name: "Front End",
    description: "Front End Front End Front End Front End Front End Front End",
    icon: {
      src: "frontendIcon",
      imgStyle: "w-16",
      initialStyle: "-translate-x-4 translate-y-12",
      translatedStyle: "-translate-x-32 translate-y-12",
    },
    dark: false,
    secondaryColor: {
      topBarBgColor: "bg-yellow",
      actionTextColor: "text-yellow",
    },
    logos: [
      {
        src: "vueLogo",
        imgStyle: "w-16",
        initialStyle: " translate-x-12 translate-y-6 rotate-45",
        translatedStyle: "translate-x-16 translate-y-6 rotate-45",
      },
      {
        src: "reactLogo",
        imgStyle: "w-32",
        initialStyle: "translate-x-16 -translate-y-10",
        translatedStyle: "translate-x-20 -translate-y-20",
      },
      {
        src: "angularLogo",
        imgStyle: "w-16",
        initialStyle: "-translate-x-8 -translate-y-8",
        translatedStyle: "-translate-x-8 -translate-y-16",
      },
    ],
  },
  {
    name: "Competetive Prog",
    description: "Competetive Prog Competetive Prog Competetive Prog",
    icon: {
      src: "frontendIcon",
      imgStyle: "w-16",
      initialStyle: "-translate-x-4 translate-y-12",
      translatedStyle: "-translate-x-32 translate-y-12",
    },
    dark: false,
    secondaryColor: {
      topBarBgColor: "bg-blue",
      actionTextColor: "text-blue",
    },
    logos: [
      {
        src: "mlLogo",
        imgStyle: "w-20",
        initialStyle: "translate-x-12 -translate-y-8",
        translatedStyle: "translate-x-20 -translate-y-16",
      },
    ],
  },
  {
    name: "Back End",
    description:
      "Back End Back End Back End Back End Back End Back End Back End",
    icon: {
      src: "frontendIcon",
      imgStyle: "w-16",
      initialStyle: "-translate-x-4 translate-y-12",
      translatedStyle: "-translate-x-32 translate-y-12",
    },
    dark: false,
    secondaryColor: {
      topBarBgColor: "bg-green",
      actionTextColor: "text-green",
    },
    logos: [
      {
        src: "laravelLogo",
        imgStyle: "w-16",
        initialStyle: "-translate-x-8 -translate-y-8",
        translatedStyle: "-translate-x-8 -translate-y-16",
      },
      {
        src: "djangoLogo",
        imgStyle: "w-20 rounded",
        initialStyle: "translate-x-12 -translate-y-6 rotate-45",
        translatedStyle: "translate-x-16 -translate-y-12 rotate-45",
      },
      {
        src: "nodeLogo",
        imgStyle: "w-16",
        initialStyle: " translate-x-16 translate-y-10",
        translatedStyle: "translate-x-20 translate-y-12",
      },
    ],
  },
  {
    name: "Online Safety Challenge",
    description: "WTM challenges WTM challenges WTM challenges WTM challenges",
    icon: {
      src: "wtmIcon",
      imgStyle: "w-16",
      initialStyle: "-translate-x-4 translate-y-8",
      translatedStyle: "-translate-x-32 translate-y-8",
    },
    dark: false,
    secondaryColor: {
      topBarBgColor: "bg-teal",
      actionTextColor: "text-teal",
    },
    logos: [
      {
        src: "onlineSafetyLogo",
        imgStyle: "w-20 ",
        initialStyle: "translate-x-12 -translate-y-8",
        translatedStyle: "translate-x-20 -translate-y-16",
      },
    ],
  },
]
